    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                # Check if scheduler is also healthy
                health_data = response.json()
                if health_data.get('scheduler', {}).get('status') == 'healthy':
                    print("✅ Airflow webserver and scheduler are ready!")
                    return True
                else:
                    print(f"⏳ Webserver ready but scheduler not healthy yet, waiting {delay}s...")
        except requests.exceptions.RequestException as e:
            print(f"⏳ Connection attempt {attempt + 1} failed: {str(e)[:50]}...")
